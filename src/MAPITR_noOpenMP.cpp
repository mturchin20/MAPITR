...
arma::mat GetLinearKernel_noOpenMP(arma::mat X){
    double p = X.n_rows;
    return X.t()*X/p;
}
...
List MAPITRBase_noOpenMP(arma::mat X,arma::mat Y,List regions){
    ...
    arma::mat GSM = GetLinearKernel_noOpenMP(X);
	...
	arma::vec y = Y.col(i);
        ...
        arma::mat K = (GSM*nsnp-GetLinearKernel_noOpenMP(X.rows(j-1))*j.n_elem)/(nsnp-j.n_elem-1);
        arma::mat G = GetLinearKernel_noOpenMP(X.rows(j-1))%K;
