...
test_that("MAPITR runs the main MAPITR function", {
	expect_equal(MAPITR_Output$Results[1,2]*100, MAPITR_Output_pValue1, tolerance=1e-6)
	expect_equal(MAPITR_Output$Results[1,3], MAPITR_Output_Est1, tolerance=1e-6)
	expect_equal(MAPITR_Output$Results[1,4], MAPITR_Output_PVE1, tolerance=1e-6)
	expect_equal(MAPITR_Output$Eigenvalues[1,1], MAPITR_Output_Eigen1, tolerance=1e-6)
	expect_equal(MAPITR_Output$Eigenvalues[1,2], MAPITR_Output_Eigen2, tolerance=1e-6)
})
