...
test_that("MAPITR runs the main MAPITR function", {
	expect_equal(MAPITR_Output$Results[1,2]*100, unname(MAPITR_Output_pValue1), tolerance=1e-8)
	expect_equal(MAPITR_Output$Results[1,3], unname(MAPITR_Output_Est1), tolerance=1e-8)
	expect_equal(MAPITR_Output$Results[1,4], unname(MAPITR_Output_PVE1), tolerance=1e-8)
	expect_equal(MAPITR_Output$Eigenvalues[1,1], unname(MAPITR_Output_Eigen1), tolerance=1e-8)
	expect_equal(MAPITR_Output$Eigenvalues[1,2], unname(MAPITR_Output_Eigen2), tolerance=1e-8)
})
