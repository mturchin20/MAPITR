...
This is a simple introductory vignette on how to run `MAPITR`. In this introductory example, we will use a simulated dataset to run `MAPITR` and explore some basic output. The purpose of this vignette is to get users up and running the `MAPIT`. The dataset is provided with the package.

## Running `MAPITR`

For this introductory vignette, we will be using `MAPITR_SimulatedPhenotype`, `MAPITR_SimulatedGenotypes`, and `MAPITR_SimulatedPathways`. These are simulated datasets created for the purposes of unit testing and this vignette. To load up 'MAPITR' and the datasets, run the following:

```{r}
library("MAPITR");

data(MAPITR_SimulatedPhenotype, MAPITR_SimulatedGenotypes, MAPITR_SimulatedPathways)
...
head(MAPITR_SimulatedPhenotype)
```
...
```{r}
MAPITR_SimulatedGenotypes[1:10,1:10]
```

```{r}
head(MAPITR_SimulatedPathways)
```

Now to run `MAPITR`, all we use is the following code:

```{r}
MAPITRResults <- MAPITR(MAPITR_SimulatedPhenotype, MAPITR_SimulatedGenotypes, MAPITR_SimulatedPathways)
```
