...
Lastly, we will go over how to incorporate covariates into this test. Our main recommendation is that most covariates should be directly regressed out from the phenotype prior to `MAPITR` analysis. In the `MAPITR` [paper][MAPITR-paper] this is how we correct for sex, age, and assessment center. The main `MAPITR` function also automatically regresses out the additive effects of each pathway from the phenotype. However, we also provide an option to remove the effects of a covariate from both the phenotype and genotypes -- specificially this is the orthogonality projection matrix, H, used to produce equation 3 from in the [MAPITR manuscript][MAPITR-paper]. This projection matrix is applied to both sides of the model, thus allowing an effect to removed both from the phenotype and the genetic relatedness matrices. By default a column of ones is included in this matrix to act as a y-intercept term. However, any additional covariates can be appended to this matrix. We currently choose to include top principal components as a part of this matrix to help remove more subtle population stratification effects from our genetic relatedness matrices.
