...
Also note that the genotype matrix can have no missing genotypes (0% rate of genotype missingness). This is necessary due to how the genetic relatedness matrices (GRMs), required components for the `MAPITR` variance component model, are constructed. For each entry in the genotype matrix, the only acceptable values are 0, 1, and 2. 
...
Lastly, we will go over how one can introduce a covariate to this test (on both sides of the model) if desired. However, we recommend that most covariates should be directly regressed out from the phenotype prior to `MAPITR` analysis. To include a covariate as part of `MAPITR` though, it is incorporated as part of the orthogonality projection matrix, H, used in equation 3 of the [MAPITR manuscript][MAPITR-paper]. This projection matrix is applied to both sides of the model, meaning it is applied to both the phenotype *and* the genotypes, and rotates both spaces to be orthogonal to the components of H. By default a column of ones is included in this matrix to act as a y-intercept term. However, any additional covariates can be appended to this matrix. We currently choose to include top principal components as a part of this matrix.

To include the top 10 principal components of our simulated dataset into the `MAPITR` analysis, we can run the following: 
```{r}
